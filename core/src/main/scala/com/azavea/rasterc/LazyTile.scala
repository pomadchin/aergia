...
import com.azavea.rasterc.LazyTile.unsafeApply
...
trait LazyTile { self =>
  ...
  @inline def cols: Int
  @inline def rows: Int
  @inline def get(col: Int, row: Int): Int = getRaw.flatMap(f => f(col, row)).value
  @inline def getDouble(col: Int, row: Int): Double = getDoubleRaw.flatMap(f => f(col, row)).value
  ...
  @inline final def dualMap(fi: Int => Int, fd: Double => Double): LazyTile =
    unsafeApply(cellType, cols, rows)(getRaw, getDoubleRaw)(fi, fd)
}

object LazyTile {
  def unsafeApply(ct: CellType, cls: Int, rws: Int)
                 (getIntF: Eval[(Int, Int) => Eval[Int]], getDoubleF: Eval[(Int, Int) => Eval[Double]])
                 (ks0: Int => Int, ks1: Double => Double): LazyTile =
    new LazyTile {
      val cellType: CellType = ct
      val cols: Int = cls
      val rows: Int = rws
      val getRaw: Eval[(Int, Int) => Eval[Int]] =
        getIntF.map { f => { (c, r) => Eval.later(f(c, r)).flatMap(_.map(ks0)) }}
      val getDoubleRaw: Eval[(Int, Int) => Eval[Double]] =
        getDoubleF.map { f => { (c, r) => Eval.later(f(c, r)).flatMap(_.map(ks1)) }}
    }
...
object LazyTest {
  ...
  def main(args: Array[String]): Unit ={
    ...
    val rr5k = timedCreate((1 to 50000).foldLeft(ltile0)((e, acc) => e + acc))
    println(rr5k.getDoubleRaw)
    println(timedCreate(rr5k.toArrayTile).asciiDraw)

    println("------------")

    val rr5k2 = timedCreate((1 to 50000).foldLeft(ltile0)((e, acc) => e + LazyTile.fromTile(IntArrayTile.fill(acc, 10, 10))))
